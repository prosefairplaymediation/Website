...
| File | Purpose |
|------|---------|
| `src/layouts/BaseLayout.astro` | HTML shell, font loading, nav, footer. Optional `noindex` prop for `/pay/agreement` and `/thank-you`. Optional `ogImage` / `ogImageAlt` props for the per-page link-preview card (see below); default is the site card `/og-image.jpg`. Delegated `book_intent_click` and `call_intent_click` GA4 listeners, on any `/book` link and any `tel:` link respectively. Optional `heroImage` prop emits `<link rel="preload" as="image" fetchpriority="high">` — set on the nine pages whose hero is a CSS `background-image`, which the browser otherwise cannot discover until the stylesheet has parsed. Only for a hero genuinely above the fold; preloading anything lower makes LCP worse. |
...
| `src/components/ConciergeCall.astro` | The Posh concierge phone promise plus a Call button, on `/` and all seven `/services` pages, at the top of the closing CTA so the call comes before the calendar. Added 2026-09-07: the concierge story appeared on one page out of twenty-eight (`/contact`) and not one service page offered a phone call at all, while the owner's position is that a real person answering every call is what sets the practice apart. **Copy is lifted verbatim from the `/contact` concierge section** — it is the only description of that service she has approved, so do not embellish it. The team schedules and explains services and pricing; it does not answer legal questions, and "concierge" never means a filing service. |
| `scripts/optimize-images.mjs` | Re-encodes the `public/` photos at their existing dimensions. **Not part of `npm run build`** — outputs are committed; run it when photos are added or replaced. Dry run by default, `--write` to apply. Never resizes and never changes format, so it cannot affect layout: the 2026-09-07 instruction was that format, fonts and colours stay the same. Quality climbs `QUALITY_LADDER` until the measured mean per-channel difference from the original is inside `MAX_MEAN_DELTA`, so it can never quietly degrade a photo to save bytes — `Notary.jpg` is why the ladder exists. First run took `public/` from 2427 KB to 1340 KB. |
