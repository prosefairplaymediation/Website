...
- **About page** at `/about` — first-person bio (V4 copy), sticky portrait of Marie (`new_Aboutme.jpg`) at native aspect with a warm gold radial aura and cream-blend overlay (the studio photo has cooler tones; the aura knits it into the cream/gold palette), credentials box ("Advanced Training in High-Conflict Resolution", Notary Public).
...
- **Contact page** at `/contact` — two-column header (text on left, portrait `New_Contact.jpg` on right). Email + phone (`(561) 941-0896`) cards, secondary booking CTA. The eFax card was removed when that number expired; the markup is kept commented in `contact.astro` for an easy restore.
...
- **Self-pay page** at `/pay` — Engagement-Agreement checkbox at top (gates the Pay buttons via a tiny vanilla-JS toggle) + one product card (Parenting Plan Preparation $600). The Turn-Key Court Packet card was removed on 2026-09-02 when the packet moved to pricing by case type. Pay buttons are `<a href>` links to Stripe Payment Links — zero backend, zero database. Entry points: secondary "Pay for a Parenting Plan" / "Pay for a Court Packet" CTAs in the respective service hero rows; an "Already agreed?" strip between hero and services on `/`; same strip between phone number and Calendly on `/landing`. Added late in scope per client request; short-circuits the consult-first model that Hourly still uses.
...
- **Stripe** — fully configured, EIN verified, bank payouts enabled. Three paid products live: **Hourly Mediation** ($600 / hour, sold in 2/4/8-hour session blocks at $1,200 / $2,400 / $4,800), **Parenting Plan Preparation** ($600 flat, raised from $400 on 2026-09-02). The **Turn-Key Court Packet** product still exists in Stripe but is no longer sold from the site: it is quoted by case type, so Marie issues a payment link once the amount is known.
